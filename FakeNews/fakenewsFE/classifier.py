...
# string to test
doc_new = ['trump is running again for president in 2020']
...
nb_pipeline.fit(DataPrep.train_news['text'], DataPrep.train_news['label'])
...
logR_pipeline.fit(
    DataPrep.train_news['text'], DataPrep.train_news['label'])
...
passive_pipeline = Pipeline([
    ...
    ('svm2_clf', PassiveAggressiveClassifier(
        loss='hinge', C=1.0, n_iter_no_change=5))
])

passive_pipeline.fit(
    DataPrep.train_news['text'], DataPrep.train_news['label'])
predicted_passive = passive_pipeline.predict(DataPrep.test_news['text'])
...
sgd_pipeline = Pipeline([
    ...
    ('svm2_clf', SGDClassifier(loss='hinge',
                               penalty='l2', alpha=1e-3,  n_iter_no_change=5))
])

sgd_pipeline.fit(
    DataPrep.train_news['text'], DataPrep.train_news['label'])
predicted_sgd = sgd_pipeline.predict(DataPrep.test_news['text'])
...
random_forest.fit(
    DataPrep.train_news['text'], DataPrep.train_news['label'])
predicted_rf = random_forest.predict(DataPrep.test_news['text'])
...
def build_confusion_matrix(classifier):
    ...
    return (print('Total texts classified:', len(DataPrep.train_news)),
...
nb_pipeline_ngram.fit(
    DataPrep.train_news['text'], DataPrep.train_news['label'])
predicted_nb_ngram = nb_pipeline_ngram.predict(DataPrep.test_news['text'])
...
pasive_pipeline_ngram = Pipeline([
    ...
    ('passive_clf', PassiveAggressiveClassifier(
        loss='hinge', C=1.0,  n_iter_no_change=5))
])

passive_pipeline_ngram.fit(
    DataPrep.train_news['text'], DataPrep.train_news['label'])
predicted_passive_ngram = passive_pipeline_ngram.predict(
    DataPrep.test_news['text'])
...
print(classification_report(
    DataPrep.test_news['label'], predicted_passive_ngram))
...
gs_clf = gs_clf.fit(
    DataPrep.train_news['text'][:10000], DataPrep.train_news['label'][:10000])
...
show_most_informative_features(
    passive_pipeline_ngram, vect='passive_tfidf', clf='passive_clf')
